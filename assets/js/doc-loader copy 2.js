...
function loadPDFs(subjectCode, category, allData) {
    ...
    items.forEach(item => {
        ...
        // ✅ Decide param: pdfid → files | id → file
        let param = "";
        if (item.pdfid) {
            param = `files=${encodeURIComponent(item.pdfid)}`;
        } else if (item.id) {
            param = `file=${encodeURIComponent(item.id)}`;
        } else {
            console.warn("No id or pdfid for item:", item);
            return;
        }

        const shareLink = `/temp/text3.html?${param}&title=${encodeURIComponent(item.title)}&subject=${encodeURIComponent(subject.name)}`;
